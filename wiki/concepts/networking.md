...
- **The vocabulary is its own**, per PRD #1093: `{ m, t, tick, d }`, six frames (`hello`, `welcome`, `refused`, `slot`, `roster`, `admission`). Keys are `m`/`t`/`d` rather than `v`/`type`/`data` so a host frame landing on the crew wire is *refused* by a decoder switching on `.type` instead of half understood. `tick` is present and unset — #1116's lockstep work cannot retrofit a stamp onto a vocabulary already in the field.
