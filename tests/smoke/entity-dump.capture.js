...
async function readHostPeerId(page) {
...
async function main() {
  ...
  await helmPage.evaluate(({ hostId, token }) => new Promise((resolve, reject) => {
    window.__messages = [];
    ...
    peer.on('open', () => {
      ...
      conn.on('data', (raw) => { try { window.__messages.push(JSON.parse(raw)); } catch {} });
    });
