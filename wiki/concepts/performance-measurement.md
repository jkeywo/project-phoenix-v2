...
The runners and validity checks live in `scripts/profile-*`; commands and interpretation are in [docs/profiling.md](../../docs/profiling.md). Build receipts pin a clean source revision, executable, symbols, SDK resources and content. Profiling builds use a target directory private to the checkout. Samples, hardware records and raw traces stay under ignored capture directories (`.phoenix/`, or `target/` for the working-tree harness).
...
The harness also supports `-Stage player` (ordinary native Helm admission) and
`-Stage multi -BridgeProfile <path>` (GM, viewscreen and player console on three
monitors). `profile_player.js` verifies live readings and retained iframe
identity. `profile-surface-summary.mjs` counts uploaded frames per visible UI
surface, separately from JS callbacks and outer window frames. Experimental
wrapper captures require their runtime source hash and matching buffered split
timings. [Ultralight tuning evidence](../../docs/acceptance/ultralight-tuning-2026-09-22.md)
records the hero-bar DOM-write improvement, worker-count comparisons and the
reverted scheduling experiment; none establishes 60 FPS embedded-UI acceptance.
