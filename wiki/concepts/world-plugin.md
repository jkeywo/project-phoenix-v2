...
- `publish_dossier_blackboard` (`src/dossier/server.rs`, added by `DossierPlugin`, `SimSet::Publish`, issue #1030) — folds every dossier subject onto the LOCAL ship's `dossiers` blackboard channel. A subject is an entity the crew already have an authoritative surface on: hailable (`CommsHailable`, #985) or publishing a condition track (`InfrastructureCondition` with `publish = true`, #1025). There is no third opt-in and no component of its own — the whole readout is derived, holds no state, and is snapshotted by nothing. The condition arrives through `InfrastructureSnapshot::from_state`, so a structure the scenario keeps off the wire is one the projection never holds the number of; the pure fold and the rule are in `src/dossier/projection.rs`.
