...
## Fleet lobby: two ship hosts on one privileged code (issue #1114)

A multi-ship session has **one** privileged code, in the `server` namespace, distinct from every ship's private crew code. It admits ship HOSTS, and nothing about it reaches the simulation.
...
- **The lead** — the host that minted the code — opens a **second** rendezvous registration in the `server` namespace beside its own crew one, and holds the slot roster. `gui/fleet-session.js`'s `createFleetOwner`/`createFleetMember` are the two ends; `gui/host-mesh.js` is the pure vocabulary and fleet model behind them.
- **The vocabulary is its own**, per PRD #1093: `{ m, t, tick, d }`. #1114's six lobby frames (`hello`, `welcome`, `refused`, `slot`, `roster`, `admission`) are revision `1`; #1116 added the running-mission `tick` and `digest` frames (revision `2`) and finally *set* the `tick` field #1114 reserved; #1119 added `host-loss` (revision `3`). Keys are `m`/`t`/`d` rather than `v`/`type`/`data` so a host frame landing on the crew wire is *refused* by a decoder switching on `.type` instead of half understood. The revision is pinned in both halves (`gui/host-mesh.js` and `src/lockstep/frame.rs`) and a receiver refuses a whole fleet of the wrong revision rather than a frame — a build that silently dropped `host-loss` would keep waiting for a peer the others had already backfilled.
...
- **Admission and freeze.** The operator's Fleet section on the settings cog closes and reopens new-host admission; closing tells admitted hosts and disconnects nobody. Mission start (the lobby phase leaving `Lobby`) freezes the roster and every selected hull, and *releases* the service-side gate rather than tightening it — `p2p-fixed-host-slot-recovery` keeps the server code a recovery capability across mission start, so a later entry must reach the lead to be judged. It is refused `recovery-only` until #1120 can honour it. A re-registration after that freeze replays the *service* gate (open), not the model's roster-level `admission` (closed), or the reconnect would silently become the lockout the freeze exists to prevent.
...
- **Where it shows.** `#fleet-panel` under the crew QR on each host's viewscreen: the letters, the invitation link and QR on the issuing host only, the slot roster and the fleet's state. Pushing the roster down to each crew's phones is not done — that needs a crew-protocol message and belongs with #1116.
- **Tests.** `tests/client/host-mesh.test.js` (envelope + fleet model), `tests/client/fleet-session.test.js` (both ends over the real registry, in process), `tests/smoke/fleet-lobby.spec.js` (two WASM host pages).
