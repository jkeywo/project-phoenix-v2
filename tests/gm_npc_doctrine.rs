...
fn inspection(app: &mut App, target: &str) -> phoenix::gm_npc::NpcDoctrineStatus {
    app.world_mut()
        .run_system_once(
            |runtime: Res<WorldContentRuntime>, control: phoenix::gm_npc::NpcDoctrineControl| {
                control.projection(&runtime)
            },
        )
        .unwrap()
        .remove(target)
...
fn checked_grant(
    ...
    sequence: u64,
    target: &str,
    choice: &phoenix::gm_npc::NpcDoctrineChoice,
) -> GmActionGrant {
    let mut result = grant(
        sequence,
        app.world().resource::<SimTick>().0,
        target,
        &choice.id,
    );
    result.action = GmAction::SetNpcDoctrineChecked {
        target: target.into(),
        doctrine: choice.id.clone(),
        expected_revision: choice.revision.clone(),
    };
    result
}
fn apply_grant(app: &mut App, grant: GmActionGrant) {
    app.world_mut()
        .resource_mut::<GmActionJournal>()
        .insert(grant)
        .unwrap();
...
#[test]
fn live_inspector_two_equal_gms_refuse_the_stale_field_and_existing_undo_restores_it() {
    let mut app = boot();
    let (npc, uuid) = named(&mut app, "Directive courier");
    let reading = inspection(&mut app, &uuid);
    let north = checked_grant(&app, 1, &uuid, &reading.choices[0]);
    let east = checked_grant(&app, 2, &uuid, &reading.choices[1]);
    apply_grant(&mut app, north);
    let behaviour = app
        .world()
    ...
    assert_eq!(facts[1].outcome, GmActionOutcome::Refused);
    assert_ne!(facts[0].operator_id, facts[1].operator_id);
    let before_after = facts[0].affected.clone().unwrap();
    // The journal's sequence is global and contiguous, while `grant` derives the
    // originating GM from sequence parity. The undo is the other equal GM's, so
    // name that origin explicitly rather than skipping a sequence to reach it.
    let mut undo = grant(3, app.world().resource::<SimTick>().0, &uuid, "north");
    undo.from = HostSlot(2);
    undo.operator_id = "gm-2".into();
    undo.order = GmActionOrder::new(undo.from, 3);
    undo.action = GmAction::UndoGmAction {
        original: GmActionId::new("npc-1").unwrap(),
        original_operator: "gm-3".into(),
        original_sequence: 1,
        expected: before_after,
    };
    apply_grant(&mut app, undo);
    assert!(app
        .world()
    ...
    let undo = app
        ...
        .last()
        .unwrap();
    assert_eq!(undo.outcome, GmActionOutcome::Applied);
    assert_eq!(undo.operator_id, "gm-2");
    assert_eq!(undo.undo_of.as_ref().unwrap().operator_id, "gm-3");
}

#[test]
fn live_inspector_definition_replacement_stales_same_id_but_unrelated_entity_changes_do_not() {
    let mut app = boot();
    let (npc, uuid) = named(&mut app, "Directive courier");
    let old = inspection(&mut app, &uuid).choices[0].clone();
    app.world_mut()
        ...
        .x += 100.0;
    assert_eq!(
        inspection(&mut app, &uuid).choices[0].revision,
        old.revision
    );
    app.world_mut()
        .resource_mut::<WorldContentRuntime>()
        .gm_npc_doctrine_palette[0]
        .doctrine[0]
        .target_speed = 0.3;
    let current = inspection(&mut app, &uuid).choices[0].clone();
    assert_eq!(current.id, old.id);
    assert_ne!(current.revision, old.revision);
    let request = checked_grant(&app, 1, &uuid, &old);
    apply_grant(&mut app, request);
    assert!(app
        .world()
        .get::<NpcDoctrineState>(npc)
        .unwrap()
    ...
    let request = checked_grant(&app, 2, &uuid, &current);
    apply_grant(&mut app, request);
    assert_eq!(doctrine(&app, npc), "north");
    let current = inspection(&mut app, &uuid).choices[0].clone();
    let request = checked_grant(&app, 3, &uuid, &current);
    apply_grant(&mut app, request);
    assert_eq!(
        app.world()
            .resource::<GmActionLog>()
...
#[test]
fn live_inspector_checks_effective_doctrine_and_refuses_withdrawn_or_missing_targets() {
    let mut app = boot();
    let (npc, uuid) = named(&mut app, "Directive courier");
    apply(&mut app, 1, &uuid, "north");
    let reading = inspection(&mut app, &uuid).choices[1].clone();
    // The palette ID is unchanged but the actual behaviour field has moved.
    app.world_mut()
        .get_mut::<BehaviourSection>(npc)
        .unwrap()
        .0
        .doctrine[0]
        .target_speed = 0.7;
    let request = checked_grant(&app, 2, &uuid, &reading);
    apply_grant(&mut app, request);
    assert_eq!(doctrine(&app, npc), "north");
    assert_eq!(
        app.world()
    ...
    app.world_mut()
        ...
        .retain(|p| p.id != "east");
    let request = checked_grant(&app, 3, &uuid, &reading);
    apply_grant(&mut app, request);
    assert_eq!(
        app.world()
            .resource::<GmActionLog>()
    ...
    let mut request = checked_grant(&app, 4, "missing-entity", &reading);
    request.action = GmAction::SetNpcDoctrineChecked {
...
#[test]
fn live_inspector_checked_readings_survive_recorded_replay_with_the_same_refusal_and_digest() {
    ...
    let reading = inspection(discovery.app_mut(), &uuid);
    let mut planned = GmActionJournal::default();
    for (index, choice) in reading.choices.iter().enumerate() {
        let mut grant = checked_grant(discovery.app_mut(), index as u64 + 1, &uuid, choice);
        grant.apply_tick = 120;
        planned.insert(grant).unwrap();
    }
    let mut recorded = drive_run_with_gm_actions(&settings, &[], &planned, 25).unwrap();
    let outcomes: Vec<_> = recorded
        .app_mut()
        .world()
        .resource::<GmActionLog>()
        .entries()
