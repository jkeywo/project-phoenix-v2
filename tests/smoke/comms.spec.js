...
test('comms — hail contact, respond, get ObjectiveSummary', async ({ context }) => {
  ...
  const initialComms = await captain.waitForMessage('CommsState', 8_000);
  const contacts = initialComms?.data?.contacts ?? [];
  ...
  await captain.page.evaluate(() => {
    window.__messages = window.__messages.filter(
      (m) => m.type !== 'CommsState',
    );
  });
  ...
  const hailComms = await captain.waitForMessage('CommsState', 8_000);
  ...
  await captain.page.evaluate(() => {
    window.__messages = window.__messages.filter(
      (m) => m.type !== 'CommsState' && m.type !== 'ObjectiveSummary',
    );
  });
  ...
  const respondComms = await captain.waitForMessage('CommsState', 8_000);
  ...
  const objSummary = await captain.waitForMessage('ObjectiveSummary', 8_000);
  const objectives = objSummary?.data?.objectives ?? [];
