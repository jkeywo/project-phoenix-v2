...
// Shape of both ledgers compared below — the committed JSON at LEDGER_PATH and
// the JSON `wasm_cross_target_probe()` returns. Neither is declared anywhere in
// this file, so the shape is written down here rather than left implicit:
//
//   { seed: number, ticks: number, interval: number, pacing: string,
//     checkpoints: { tick: number, digest: string }[], final_digest: string }
//
// The wasm side may additionally carry `error: string` when the probe refused
// to run; the test checks for that explicitly before comparing digests.

/**
 * The first tick at which two ledgers disagree — the JS twin of
...
test('wasm: a delayed browser instance folds the same digests as the native pin', async ({
  ...
  const json = await page.evaluate(
    () => window.wasm_cross_target_probe(),
  );
