...
fn low_fidelity_capture_replaces_a_high_fidelity_bootstrap_bundle() {
    ...
    use project_phoenix::ai::server::{
        ai_high_fidelity_components, AiHighFidelity, AiHighFidelityComponents, LodTransitionTimer,
    };
    ...
    let (source_entity, source_uuid) = {
        let world = live.world_mut();
        let mut query = world
            .query_filtered::<(Entity, &EntityUuid), (With<AiHighFidelity>, Without<LocalShip>)>();
        query
            .iter(world)
            .next()
            .map(|(entity, uuid)| (entity, uuid.0.clone()))
    ...
    let assert_low = |app: &bevy::prelude::App, context: &str| {
        ...
        assert!(
            world
                .get::<ShipFrequencyHintState>(bootstrap_entity)
                .is_none(),
        ...
        assert!(
            world.get::<LodTransitionTimer>(bootstrap_entity).is_none(),
...
/// The inverse lifecycle mismatch is equally authoritative. A captured High
/// ship restored over a Low bootstrap must receive the whole canonical bundle
/// before its stored actuator and policy continuation is applied; otherwise
/// each `get_mut` write silently skips the component that was absent.
#[test]
fn high_fidelity_capture_replaces_a_low_fidelity_bootstrap_bundle() {
    use bevy::prelude::{Entity, Fixed, Time, With, Without};
    use project_phoenix::ai::server::{
        AiHighFidelity, AiHighFidelityComponents, LodTransitionTimer,
    };
    use project_phoenix::console_ai::{server::ShipFrequencyHintState, FrequencyHintState};
    use project_phoenix::entities::spawner::EntityUuid;
    use project_phoenix::server_app::{LocalShip, ShipBoost, ShipImpulse};
    use project_phoenix::ship::helm::{SteeringInput, ThrustInput};
    use project_phoenix::ship::helm_ai::{HelmBoostAiPolicyState, HelmPassSurface};
    use project_phoenix::ship::impulse::ImpulsePhase;
    ...
    let (source_entity, source_uuid) = {
        ...
        query
            ...
            .next()
            .map(|(entity, uuid)| (entity, uuid.0.clone()))
            .expect("the live duel has a high-fidelity NPC")
    };
    let captured_at = f64::from(live.world().resource::<Time<Fixed>>().elapsed_secs());
    {
        let mut ship = live.world_mut().entity_mut(source_entity);
        ship.get_mut::<ThrustInput>()
            .expect("the High bundle carries thrust")
            .0 = 0.731;
        ship.get_mut::<SteeringInput>()
            .expect("the High bundle carries steering")
            .0 = -0.42;
        let mut drive = ship
            .get_mut::<ShipBoost>()
            .expect("every ship carries the boost runtime");
        drive.0.active = true;
        drive.0.battery = 0.375;
        let mut impulse = ship
            .get_mut::<ShipImpulse>()
            .expect("every ship carries the impulse runtime");
        impulse.0.phase = ImpulsePhase::Charging;
        impulse.0.charge_progress = 0.625;
        ship.get_mut::<ShipFrequencyHintState>()
            .expect("the High bundle carries frequency continuation")
            .0 = FrequencyHintState {
            current_target: Some("fidelity-target".into()),
            elapsed_secs: 0.875,
            hint_sent: true,
        };
        ship.insert(LodTransitionTimer {
            last_state_change_secs: captured_at - 0.75,
        });
    }
    ...
    let source_row = payload
        ...
        .iter()
        .find(|row| row.uuid == source_uuid)
        .expect("the High NPC has a snapshot row")
        .clone();
    assert_eq!(
        source_row
            .ai_fidelity
            .as_ref()
            .map(|state| state.high_fidelity),
        Some(true)
    );
    ...
    resumed
        .world_mut()
        .entity_mut(bootstrap_entity)
        .remove::<AiHighFidelityComponents>();
    assert!(
        resumed
            .world()
            .get::<AiHighFidelity>(bootstrap_entity)
            .is_none(),
        "precondition: the destination begins Low"
    );

    let report = restore(resumed.world_mut(), &payload);
    assert!(report.is_complete(), "gaps: {:?}", report.gaps);
    let restored = resumed.world().entity(bootstrap_entity);
    assert!(restored.get::<AiHighFidelity>().is_some());
    assert!(restored.get::<HelmBoostAiPolicyState>().is_some());
    assert!(restored.get::<HelmPassSurface>().is_some());
    let restored_row = capture(resumed.world())
        .entities
        .into_iter()
        .find(|row| row.uuid == source_uuid)
        .expect("the restored High NPC is recaptured");
    assert_eq!(restored_row.ai_fidelity, source_row.ai_fidelity);
    assert_eq!(restored_row.control, source_row.control);
    assert_eq!(restored_row.drive, source_row.drive);
    assert_eq!(restored_row.frequency_hint, source_row.frequency_hint);
    assert_eq!(restored_row.pass_surface, source_row.pass_surface);

    step(&mut live, 1);
    step(&mut resumed, 1);
    let live_drive = capture(live.world())
        .entities
        .into_iter()
        .find(|row| row.uuid == source_uuid)
        .and_then(|row| row.drive)
        .expect("the live drive remains captured");
    let resumed_drive = capture(resumed.world())
        .entities
        .into_iter()
        .find(|row| row.uuid == source_uuid)
        .and_then(|row| row.drive)
        .expect("the resumed drive remains captured");
    assert_eq!(resumed_drive, live_drive);
}

/// A stale marker is not proof that the destination carries the canonical
/// High bundle. Restore must repair a partially constructed bootstrap before
/// applying continuation, or the missing component's write is silently lost.
#[test]
fn high_fidelity_capture_repairs_a_partial_bootstrap_bundle() {
    use bevy::prelude::{Entity, With, Without};
    use project_phoenix::ai::server::{
        ai_high_fidelity_components, has_ai_high_fidelity_components, AiHighFidelity,
    };
    use project_phoenix::console_ai::{server::ShipFrequencyHintState, FrequencyHintState};
    use project_phoenix::entities::spawner::EntityUuid;
    use project_phoenix::server_app::LocalShip;
    ...
    live.world_mut()
        ...
        .get_mut::<ShipFrequencyHintState>()
        .expect("the canonical bundle carries the hint timer")
        .0 = FrequencyHintState {
        current_target: Some("partial-bundle-target".into()),
        elapsed_secs: 1.25,
        hint_sent: true,
    };
    let payload = capture(live.world());
    let source_hint = payload
        .entities
        .iter()
        .find(|row| row.uuid == source_uuid)
        .and_then(|row| row.frequency_hint.clone())
        .expect("the hint continuation is captured");
    ...
    resumed
        ...
        .insert(ai_high_fidelity_components())
        .remove::<ShipFrequencyHintState>();
    assert!(
        resumed.world().get::<AiHighFidelity>(destination).is_some(),
        "precondition: the stale High marker remains"
    );

    let report = restore(resumed.world_mut(), &payload);
    assert!(report.is_complete(), "gaps: {:?}", report.gaps);
    {
        let destination_ref = resumed.world_mut().entity_mut(destination);
        assert!(has_ai_high_fidelity_components(&destination_ref));
    }
    let restored_hint = capture(resumed.world())
        .entities
        .into_iter()
        .find(|row| row.uuid == source_uuid)
        .and_then(|row| row.frequency_hint)
        .expect("the repaired component receives its captured continuation");
    assert_eq!(restored_hint, source_hint);
}

/// `lod_ai_ships` demotes on `age >= 2.0`, so moving the timestamp by even one
/// representable value changes the first eligible tick. Snapshot format 14
/// stores the age and restore must rebase that exact boundary onto the fresh
/// process clock, not copy the source process's absolute timestamp.
#[test]
fn ai_fidelity_timer_rebases_exact_dwell_boundary() {
    use bevy::prelude::{Entity, Fixed, Time};
    use project_phoenix::ai::lod::{evaluate_lod, LodState};
    use project_phoenix::ai::server::{AiHighFidelity, LodTransitionTimer};
    use project_phoenix::entities::spawner::EntityUuid;
    ...
    let (source_entity, source_uuid) = {
        let world = live.world_mut();
        let mut query = world.query::<(Entity, &EntityUuid, &AiHighFidelity)>();
        query
            .iter(world)
            .next()
            .map(|(entity, uuid, _)| (entity, uuid.0.clone()))
            .expect("the duel has a High-fidelity ship")
    };
    live.world_mut()
        .entity_mut(source_entity)
        .insert(LodTransitionTimer {
            last_state_change_secs: captured_at - 2.0,
        });
    let payload = capture(live.world());
    assert_eq!(
        payload
            ...
            .iter()
            .find(|row| row.uuid == source_uuid)
            .and_then(|row| row.ai_fidelity.as_ref())
            .and_then(|state| state.last_transition_age_secs),
        Some(2.0),
        "capture records the exact inclusive dwell boundary"
    );
    ...
    let restored_entity = {
        ...
        query
            .iter(world)
            .find_map(|(entity, uuid)| (uuid.0 == source_uuid).then_some(entity))
            .expect("the bootstrap has the boundary ship")
    };
    let report = restore(resumed.world_mut(), &payload);
    assert!(report.is_complete(), "gaps: {:?}", report.gaps);

    let restored_at = f64::from(resumed.world().resource::<Time<Fixed>>().elapsed_secs());
    let timer = resumed
        .world()
        .get::<LodTransitionTimer>(restored_entity)
        .expect("the stored timer presence is restored");
    assert_eq!(timer.last_state_change_secs, restored_at - 2.0);
    assert!(resumed
        .world()
        .get::<AiHighFidelity>(restored_entity)
        .is_some());
    assert_eq!(
        evaluate_lod(
            LodState::High,
            ...
            600.0,
            restored_at,
            timer.last_state_change_secs,
            2.0,
            0.2,
        ),
        LodState::Low,
        "the rebased age lands on the inclusive dwell boundary"
    );
}

/// `SimTick` alone does not identify where the rendered update sits relative
/// to the next fixed step. The accumulator must survive as integer nanos so an
/// exact sub-step boundary cannot be rounded through floating-point seconds.
#[test]
fn fixed_overstep_round_trips_exactly_and_continues() {
    use bevy::prelude::{Fixed, Time};
    use std::time::Duration;

    const OVERSTEP_NANOS: u64 = 3_456_789;

    let mut live = duel();
    step(&mut live, CAPTURE_AT);
    {
        let mut fixed = live.world_mut().resource_mut::<Time<Fixed>>();
        let stale = fixed.overstep();
        fixed.discard_overstep(stale);
        fixed.accumulate_overstep(Duration::from_nanos(OVERSTEP_NANOS));
    }
    let payload = capture(live.world());
    assert_eq!(payload.fixed_overstep_nanos, Some(OVERSTEP_NANOS));
    ...
    assert_eq!(
        resumed
            ...
            .overstep()
            .as_nanos(),
        u128::from(OVERSTEP_NANOS)
    );

    step(&mut live, 1);
    step(&mut resumed, 1);
    assert_eq!(capture(resumed.world()).tick, capture(live.world()).tick);
    assert_eq!(world_digest(resumed.world()), world_digest(live.world()));
}

/// Two first-continuation gates live outside the component values restored by
/// the older payload: damage-offline command admission and the projection
/// registry that decides whether to rewrite `WorldResource`. Both must replace
/// bootstrap state exactly before the first update.
#[test]
fn offline_admission_and_tracked_registry_replace_bootstrap_state() {
    use bevy::prelude::{Entity, Without};
    ...
    use project_phoenix::server_app::{LocalShip, TrackedEntities};
    ...
    let (source_entity, source_uuid) = {
        let world = live.world_mut();
        let mut query = world
            .query_filtered::<(Entity, &EntityUuid, &ShipSystemControlSources), Without<LocalShip>>(
            );
        query
            .iter(world)
            .next()
            .map(|(entity, uuid, _)| (entity, uuid.0.clone()))
            .expect("the duel has a ship with a command-admission resolver")
    };
    let captured_offline = SystemId("helm-thrust".into());
    let bootstrap_only_offline = SystemId("helm-steering".into());
    live.world_mut()
        .entity_mut(source_entity)
        .get_mut::<ShipSystemControlSources>()
        .expect("the source resolver remains attached")
        .0
        .replace_offline_systems([captured_offline.clone()]);
    {
        let mut tracked = live.world_mut().resource_mut::<TrackedEntities>();
        tracked.reported.insert("captured-reported-entity".into());
        tracked.seeded = true;
    }
    let payload = capture(live.world());
    let expected_tracked = payload
        .tracked_entities
        .clone()
        .expect("the projection registry is captured");
    assert_eq!(
        payload
            .entities
            .iter()
            .find(|row| row.uuid == source_uuid)
            .map(|row| row.damage_offline_systems.clone()),
        Some(vec![captured_offline.0.clone()])
    );
    ...
    resumed
        ...
        .entity_mut(destination)
        .get_mut::<ShipSystemControlSources>()
        .expect("the destination resolver remains attached")
        .0
        .replace_offline_systems([bootstrap_only_offline.clone()]);
    *resumed.world_mut().resource_mut::<TrackedEntities>() = TrackedEntities::default();

    let report = restore(resumed.world_mut(), &payload);
    assert!(report.is_complete(), "gaps: {:?}", report.gaps);
    let restored = capture(resumed.world());
    assert_eq!(restored.tracked_entities, Some(expected_tracked));
    assert_eq!(
        restored
            .entities
            .iter()
    ...
    );
    {
        ...
        let ship = world.entity(destination);
        let sources = ship
            .get::<ShipSystemControlSources>()
            .expect("the restored ship has its production admission resolver");
        let config = ship
            .get::<ShipConfigComponent>()
            .expect("the restored ship has its authored system topology");
        let sessions = world.resource::<Sessions>();
        assert!(config.0.system(&captured_offline).is_some());
        assert!(config.0.system(&bootstrap_only_offline).is_some());
        assert!(
            !is_command_authorized(
                "ai:snapshot-offline-probe",
                &captured_offline,
            ...
            "the captured damage-offline system is rejected at production admission before damage sync"
        );
        assert!(
            is_command_authorized(
                "ai:snapshot-offline-probe",
                &bootstrap_only_offline,
            ...
            "restore clears the bootstrap-only offline id before production admission"
        );
    }
...
fn navigation_continuation_round_trips_anchor_and_exact_generations() {
    ...
    assert_eq!(row.helm_waypoint_clearance.as_ref(), Some(&helm_clearance));
}
...
fn default_navigation_continuation_replaces_non_default_bootstrap() {
    ...
    let seeded_row = seeded
        ...
        .expect("the seeded bootstrap ship is captured");
    assert_eq!(
        seeded_row.navigation_waypoint.as_ref(),
        Some(&seeded_waypoint)
    );
    assert_eq!(
