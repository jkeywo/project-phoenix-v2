...
// and explosion-pfx.capture.js. Run it explicitly:
//
//   npx playwright test --config=playwright.capture.config.js torpedo-pfx.capture.js --headed
//
  ...
  await tactical.page.waitForFunction(
    () => window.__messages?.some(
      (m) => m.type === 'WeaponsUpdate'
        ...
        && m.data.tubes.some((t) => t.id === 'fore_port' && t.loaded_count > 0),
    ),
    undefined,
    { timeout: 15_000 },
  );
