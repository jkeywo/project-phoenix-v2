...
// designer happens to have attached. See the header of `fixtures.ts` for the
// fixture convention.
...
description = "Player ship + one NPC with a GLB; see tests/smoke/ship-mesh-load.spec.ts."
...
test.skip('NPC ship GLB model loads (200) after game start', async ({ context }) => {
  ...
  const modelFile = modelPath.split('/').pop() as string;
