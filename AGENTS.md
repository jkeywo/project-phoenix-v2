...
src/
  ...
  world/
    server.rs         — WorldPlugin: world-file loading, entity lifecycle, trigger evaluation,
                        ...
                        `spawn_world_entities`, `init_world_runtime`, and `setup_fallback_world`
                        (PRD #341 — single `.chain()` startup; fallback gated by
                        `run_if(not(resource_exists::<WorldConfig>))`).
    config.rs         — Pure single-pass parser (`parse_world` → `WorldConfig`) for the unified
                        world TOML. Owns anchors (normalised `[f32;3]`), `[[entity]]` instances,
                        `[[trigger]]` blocks, and `[[comms]]` templates (PRD #341). Helpers
                        `entity_template_paths` (dedup queue) and `partition_immediate_entities`
                        (asteroid-field vs. other routing). No Bevy systems, no serde_json —
                        only `toml::from_str`.
...
assets/
  ...
  entities/player_ship.toml     — Ship config: helm_console physics, phaser banks (`[[weapons_console.phaser_banks]]` data-driven; each has `id`/`facing_deg`/`fire_arc_deg`/`auto_arc_deg`; `[weapons_console]` block tunes shared `beam_range`/`beam_damage_per_sec`/`beam_duration_secs`/`cooldown_secs`/`beam_color`/`torpedo_arc_color`), torpedo tubes (`[[torpedoes.tubes]]` data-driven with `id`/`facing_deg`/`fire_arc_deg`; ammo is a shared pool via `[torpedoes].count`; `[torpedoes]` tunes damage/speed/turn_rate/lifespan/load_time), [repair] block tunes repair-team travel duration + repair rate (also broadcast to client via ShipClientConfig), shields ([shields_console.base] block tunes num_facings/max_hp/regen_per_sec/offline_duration), impulse, [stations].
...
- **Helm:** Thrust + steering joystick. Sends `HelmInput` at 10Hz while active. Ship only moves when Helm is occupied. Displays radar overlay and "On Screen" button to push radar to the viewscreen. Triggers impulse charge via `StartImpulseCharge`. While impulse is Charging or Active the joystick is hidden behind a charging-progress bar + `CANCEL IMPULSE` button + `x.x / y.y s` status text (`ENGAGED` when Active); the server forces `thrust=1.0, steering=0.0` and multiplies `ShipPhysicsConfig.acceleration` by `[helm_console].impulse_acceleration_multiplier` (default 5.0). `[helm_console].impulse_speed_multiplier` (default 10.0) drives the `MaxSpeed` modifier via `ImpulseConfigResource`; both multipliers are TOML-tunable, and `impulse_charge_duration` is shipped to the client on `Welcome` via `ShipClientConfig.impulse_charge_duration`.
