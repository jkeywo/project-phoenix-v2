...
test('audio config is data-driven from ship + world TOML and builds the audio graph', async ({
  ...
  expect(dbg.cfg.ambient.file).toBe('assets/sounds/Ambient.mp3');
  expect(dbg.cfg.engine.file).toBe('assets/sounds/Engine.mp3');
  expect(dbg.cfg.blaster.file).toBe('assets/sounds/Blaster.mp3');
  expect(dbg.cfg.phaser_loop.file).toBe('assets/sounds/PhaserLoop.mp3');
  expect(dbg.cfg.forcefield.file).toBe('assets/sounds/ForcefieldHit.mp3');

  // Red-alert audio comes from the *world* TOML, not the ship.
  expect(dbg.cfg.red_alert.siren_file).toBe('assets/sounds/red_alert_siren.ogg');
  expect(dbg.cfg.red_alert.music_file).toBe('assets/sounds/last_stand_in_space_looped.ogg');
  ...
  // The new files are actually fetched.
  await serverPage.waitForFunction(
    () => performance.getEntriesByType('resource').some((e) => /Ambient\.mp3/.test(e.name)),
    undefined,
    { timeout: 10_000 },
  );
  expect(audioRequests).toEqual(expect.arrayContaining(['Ambient.mp3']));
  ...
  // The authored ambient volume (0.25) must survive the trip from TOML to
  // the element — this is the value the JS used to hardcode.
  expect(dbg.volumes.ambient).toBeCloseTo(0.25, 5);
  // Engine starts at its idle volume until thrust rides it up.
  expect(dbg.volumes.engine).toBeCloseTo(0, 5);
  ...
  expect(firing.volumes.phaser).toBeCloseTo(0.5, 5);
  ...
  // (idle 0.0 + thrust * 0.15), not a hardcoded constant.
  const thrusting = await serverPage.evaluate((s) => {
    (window as any).__updateHud(JSON.stringify(s));
    return (window as any).__audioDebug();
  }, hud({ engine_thrust: 1.0 }));
  expect(thrusting.volumes.engine).toBeCloseTo(0.15, 5);

  const halfThrust = await serverPage.evaluate((s) => {
    (window as any).__updateHud(JSON.stringify(s));
    return (window as any).__audioDebug();
  }, hud({ engine_thrust: 0.5 }));
  expect(halfThrust.volumes.engine).toBeCloseTo(0.075, 5);
