...
assets/
  ...
  entities/player_ship.toml     — Ship config: helm_console physics, phaser banks ([weapons_console] beam_range/beam_damage_per_sec/beam_duration_secs/cooldown_secs tune live player phasers via PhaserCombatConfigResource), torpedo tubes ([torpedoes] block tunes count/damage/speed/turn_rate/lifespan/load_time), [repair] block tunes repair-team travel duration + repair rate (also broadcast to client via ShipClientConfig), shields ([shields_console.base] block tunes num_facings/max_hp/regen_per_sec/offline_duration), impulse, [stations].
