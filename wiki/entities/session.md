...
Physical connections on both host targets are owned separately by
...
The browser constructs `BrowserConnections` from the WASM module before
opening crew admission, independently of ECS startup. JS holds physical
connections by opaque handle; pre-World picks and buffered input check that
handle's current sender. The registry persists through World selection.
