...
try {
  ...
  await page.waitForFunction(() => window.wasmBindings?.viewer_load_entity, { timeout: 180000 });
  ...
  for (const [name, yaw, pitch, radius, sunYaw, sunPitch] of [
    ...
    }, { yaw, pitch, radius, sunYaw, sunPitch });
  ...
  const report = { errors, stats: JSON.parse(await page.evaluate(() => window.wasmBindings.viewer_stats())) };
