...
- The server page (`server.html`) registers with the rendezvous service on every load and is issued a **private typed code** in the client namespace. The join panel prints that code and a QR of `https://…/client/index.html#<PROJECT_GUID>_<VERSION_GUID>_<CODE>` — the same code, so scanning and typing are one join by two routes.
- Client pages either read that structured code out of `location.hash` or take the eight typed letters; `gui/join-code.js` decides which a given string is. Both resolve through the service to the same host record and open the same channels.
...
- **Join identifiers** are `PROJECT_GUID_VERSION_GUID_CODE`, where `CODE` is **eight** letters over a 25-letter alphabet — 25^8 ≈ 1.5 × 10^11, 37.15 bits, because the code is the only secret in front of a game and five letters (23.2 bits) were walkable in about 35 minutes by a caller willing to reconnect (see the `[suffix]` note in the authored table). `gui/join-code.js` is the pure scheme — canonicalisation (upper-case; `0`→`O`, `1`/`L`→`I`, `J` distinct), typed validation, deny-list refusal (a code is refused for *containing* an authored word, not only for being one), compose/parse and minting — reading its alphabet, GUIDs and deny-list from the authored `assets/join/join-codes.toml`, through `assets/join/join-codes.json`, a committed artifact `scripts/build-client.mjs` regenerates for the three consumers that have no TOML parser (the phone, vitest, the Worker bundle). Client and server joining have separate project GUIDs, so a fleet code typed into the crew field is a *wrong-type* answer rather than a miss.
...
- **Deploy trap:** a stale `ALLOWED_ORIGIN` prevents browser/cloud joining through the rendezvous service. `/v1/health` echoes `origin_allowed` for that check — see `docs/delivery-checklist.md` §3a. Native LAN joining uses the host's own direct-join service and does not require the Cloudflare service; its optional cloud leg still does.
...
- **Frozen crew Ratings.** `SessionManager::lobby_station_ratings` publishes the connected non-Spectator holders' pending Station choices without names or session tokens. `gui/fleet-crew.js` validates and copies exact Station/Rating pairs; `gui/fleet-session.js` carries them with readiness, including pre-Welcome choices and Rating-only changes. The owner freezes them into `FleetShip.crew`. `src/lockstep/crew.rs` checks the bounded pairs against each selected hull before fresh adoption changes the clock or roster. Every fleet hull, including the local one, therefore boots from the same crewing facts on every host.
...
Commit, preserving existing ledgers and disagreement history. A private joining
candidate has no authoritative sampler until that boundary. The M1 browser
trace checks actual same-tick digests after the final crew transition.
...
Both host targets use `session_connections::ConnectionRegistry` for physical
ownership, apart from authoritative Session state. The browser constructs its
`BrowserConnections` WASM wrapper when module exports arrive and waits for it
before registering the crew transport. No World or ECS App is needed, so phones
can Identify and make the early scenario/hull picks.

The JS adapter in `gui/host-peer-routing.js` keeps physical connections by
opaque incarnation handle. Rust binds accepted Identify, refuses identity
changes and invalid/reserved tokens, replaces ownership before old close, and
selects current recipients. Every input and departure checks that same owner;
queued pre-ECS messages recheck their handle at flush. The snapshot channel is a
property of its physical connection, so a late old channel event cannot overwrite
the new connection's route.
...
`server.html` asks `createHostConnections` in
`gui/host-peer-routing.js` for the Rust registry's current recipients. The
physical adapter prefers the lossy channel for `snapshot` and
...
re-types the code. During a round, registered replication owners reconstruct
...
simply be reading the wrong code. After acceptance it is unbounded.
