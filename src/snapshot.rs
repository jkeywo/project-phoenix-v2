...
use crate::server_app::{
    AsteroidUuid, CaptainPriorityBoost, GameOverReason, ShipBoost, ShipImpulse,
};
...
/// Format 14 also carries `Time<Fixed>`'s exact nanosecond overstep. Removing
/// `ThrustInput` from restore readiness lets a Low bootstrap become ready on a
/// different rendered frame, and without the accumulator the first resumed
/// `App::update()` can run zero fixed steps while the live world runs one.
/// The same boundary explicitly carries every ship's High/Low fidelity tier
/// and transition age, the canonical bundle's outside drive runtime, the
/// damage-offline control gate, and the broadcast projection registry. These
/// are one lifecycle repair: changing readiness exposes bootstrap states that
/// used to settle before restore, and each is read before its ordinary repair
...
pub struct EntityState {
    ...
    /// recovered in time from the restored hull: command admission reads it
    /// before the next damage-sync pass. The ids are sorted for stable saves;
    ...
    /// The impulse/boost drive machines actuated by the helm commands above.
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
/// Compatibility path for the unchanged saved Power payload. The reactor owns
/// its projection and conversion; this module owns traversal and restore order.
pub use crate::modifiers::power_system::PowerState;

/// Compatibility paths for the unchanged saved control payloads. Component
/// conversion belongs to the Ship; this module retains identity joins and order.
pub use crate::ship::continuation::{ControlState, PolicyState, RecoveryHistory};

/// Runtime state of the two helm drives outside the High-fidelity bundle.
///
/// `BoostCommand` and `ImpulseCommand` are desired transitions; these are the
/// battery/charge machines integration actually reads. They must travel
/// separately because inserting a High bundle marks its commands as Added,
/// which the normal transition system intentionally ignores on promotion.
#[derive(Clone, Debug, Default, PartialEq, Serialize, Deserialize)]
...
pub struct PhoenixSnapshot {
    ...
    /// update is already part-way toward another one. It is stored as integer
    /// nanoseconds because Bevy's accumulator is a `Duration`, and projecting it
    ...
    /// The non-asteroid entity projection registry used by reconnect/broadcast.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub tracked_entities: Option<TrackedEntitiesState>,
...
#[derive(Clone, Debug, Default, PartialEq, Serialize, Deserialize)]
pub struct TrackedEntitiesState {
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
...
pub fn capture(world: &World) -> PhoenixSnapshot {
    ...
    PhoenixSnapshot {
        ...
        tracked_entities: world
            .get_resource::<crate::server_app::TrackedEntities>()
            .map(|tracked| {
                let mut reported: Vec<String> = tracked.reported.iter().cloned().collect();
                reported.sort();
                TrackedEntitiesState {
                    reported,
                    seeded: tracked.seeded,
                }
            }),
        flags: world
...
fn capture_scenario(world: &World) -> Option<ScenarioState> {
    ...
    Some(ScenarioState {
        ...
        contact_information: runtime.contact_information.clone(),
...
///
/// A row is emitted for every ship. That includes a never-promoted Low NPC (no
/// marker, no timer), a demoted NPC (timer but no marker), and the permanently
/// High local ship. Querying the domain marker instead of inferring ship-ness
...
fn restore_run_scope(world: &mut World, snapshot: &PhoenixSnapshot, report: &mut RestoreReport) {
    ...
    if let (Some(stored_nanos), Some(mut fixed)) = (
        ...
        // Replace, do not add to, whatever fraction the bootstrap accumulated.
        // Both operations are exact `Duration` arithmetic; no seconds/f32
        // projection is allowed at this fixed-step boundary.
    ...
    if let Some(stored) = &snapshot.tracked_entities {
        world.insert_resource(crate::server_app::TrackedEntities {
            reported: stored.reported.iter().cloned().collect(),
...
fn restore_scenario(world: &mut World, snapshot: &PhoenixSnapshot, report: &mut RestoreReport) {
    ...
    if let Some(mut runtime) = world.get_resource_mut::<WorldContentRuntime>() {
        ...
        runtime.presentation = stored.presentation.clone();
        runtime.contact_information = stored.contact_information.clone();
...
fn apply_navigation_waypoint(entity: &mut EntityWorldMut<'_>, stored: &NavigationWaypointState) {
    ...
    if let Some(mut waypoint) = entity.get_mut::<crate::console::navigation::NavigationWaypoint>() {
...
fn apply_navigation_clearance_issue(
    ...
    if let Some(mut state) = entity.get_mut::<crate::console::navigation::NavClearanceIssueState>()
    {
...
fn apply_helm_waypoint_clearance(
    ...
    if let Some(mut clearance) = entity.get_mut::<crate::ship_plugin::HelmWaypointClearance>() {
        clearance.0 = stored.generation;
    } else {
        entity.insert(crate::ship_plugin::HelmWaypointClearance(stored.generation));
    }
}
...
/// captured fixed-clock age. A complete High bundle and a present timer are
/// updated in place: removing and reinserting an already-correct bundle would
/// make an unnecessary archetype round-trip, and archetype iteration order is
/// itself determinism-sensitive (see `AiHighFidelity`'s write-guard history).
fn apply_ai_fidelity(entity: &mut EntityWorldMut<'_>, stored: &AiFidelityState, restored_at: f64) {
    use crate::ai::server::{
        ai_high_fidelity_components, AiHighFidelityComponents, LodTransitionTimer,
    };

    if stored.high_fidelity {
        if !crate::ai::server::has_ai_high_fidelity_components(entity) {
            // Inserting the complete tuple replaces every present partial
            // member with its canonical default before the continuation writes
            // below restore the captured values.
    ...
    match stored.last_transition_age_secs {
        Some(age) => {
            let last_state_change_secs = restored_at - age;
            if let Some(mut timer) = entity.get_mut::<LodTransitionTimer>() {
                timer.last_state_change_secs = last_state_change_secs;
            } else {
                entity.insert(LodTransitionTimer {
                    last_state_change_secs,
                });
            }
        }
        None => {
...
pub fn ready_to_rebuild(world: &World, snapshot: &PhoenixSnapshot) -> bool {
    ...
    let present: Vec<&str> = query.iter(world).map(|uuid| uuid.0.as_str()).collect();
    let roster_rebuildable = snapshot
        .entities
        .iter()
        .all(|row| present.contains(&row.uuid.as_str()) || row.spawn.is_some());
