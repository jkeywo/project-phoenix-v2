...
//! (`process_helm_inputs`, `handle_boost_messages`) has already been validated
...
/// (issue #833): `process_helm_inputs` applies the four per-axis helm ids plus
/// vertical thrust in one applier, and `handle_boost_messages` applies
/// `helm-boost`. Called from `ShipPlugin::build`.
///
