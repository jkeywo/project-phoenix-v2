...
test('audio config is data-driven from ship + world TOML and builds the audio graph', async ({
  ...
  serverPage.on('request', (r) => {
    const u = r.url();
    if (/assets\/sounds\//.test(u)) audioRequests.push(u.split('/').pop());
  });
  ...
  expect(await serverPage.evaluate(() => window.__audioDebug().cfg)).toBeNull();
  ...
  await serverPage.waitForFunction(() => !!window.__audioDebug().cfg, undefined, {
    timeout: 10_000,
  });
  ...
  const alertOn = await serverPage.evaluate((s) => {
    window.__updateHud(JSON.stringify(s));
    return window.__audioDebug();
