...
(function () {
  ...
  const operatorRecord = window.PhoenixInstallNativeOperatorStorage(
    record => window.phoenixPaneOut.send(record));
  window.PhoenixNativeGamepad = {
    select: index => operatorRecord('select', {index}),
  };
  ...
  window.__phoenixPaneOutDrain = () => {
    const records = drain();
    const audio = window.__phoenixPrivateAudioDrain?.();
    return [records, audio].filter(Boolean).join('\n');
  };
})();
