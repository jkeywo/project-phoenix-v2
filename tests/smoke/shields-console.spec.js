...
test('shields console: variable arc count renders correctly', async ({ page }) => {
  await page.goto(CONSOLE_URL);
  await page.evaluate((s) => window.__updateConsole('shields', JSON.stringify(s)), {
...
test('shields console: threat indicator hidden when no target', async ({ page }) => {
  await page.goto(CONSOLE_URL);
  await page.evaluate((s) => window.__updateConsole('shields', JSON.stringify(s)), { ...NOMINAL_STATE, threat_bearing: null });
