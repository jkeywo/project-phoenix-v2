...
async function startGameWithEngineering(context) {
...
test('SystemHullUpdate is projected per recipient, not broadcast whole (#737)', async ({ context }) => {
  ...
  const engMsg = await engineer.waitForMessage('SystemHullUpdate', 5_000);
  const helmMsg = await captain.waitForMessage('SystemHullUpdate', 5_000);

  const rowIds = (m) =>
    m.data.entries.map((e) => e.system_id).sort();
