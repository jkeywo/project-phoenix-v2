...
| `gm-local-projection.js`, `entity-inspector.js`, `components/ph-navigation-map.js` inspect mode | Strict local Host Channel GM map DTO, stable UUID selection across absolute refresh/removal, the reusable M6 identity/status/target inspector shell, and shared touch/keyboard/pan/zoom map presentation without a peer state stream or Navigation command |
