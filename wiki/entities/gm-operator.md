...
activity, session and Station-puppeting presentations and typed action reducers
...
Issue #1490 adds the world/scenario domain on the same reading contract. The
root reading combines effective parsed `[global]` values with the mission
event, Objective, deadline and scenario-state owners already used elsewhere on
the desk. Each active supporting layer retains the effective definition used
to load it and receives its own origin-tagged runtime records. Scenario Flags
are derived values only. Fire/Pause/Skip, Objective and
session rows point to their existing dock panels, so the Inspector adds no
second action lifecycle and no generic field setter. Supporting-world paths are
...
Issue #1492 adds the active Region domain. Its explicit schema ratchet covers
identity and retained template/layer provenance, all translation/rotation/scale
axes, every sphere/box/torus field, every Region effect field and presence, and
the complete radar presentation section. Authored topology is Recreate required
with the reason displayed beside the value. `RegionMembership` contributes only
stable hull identity and its public label; the selected Region's effective
damage, motion, radar, impulse, comms, sensor and fog consequences are derived
from public effect components, never from a ship's private modifier cache or
crew/session state. Unloading or removing a Region freezes its last reading by
stable entity id as gone. Selecting a live non-Region clears the panel, so it
cannot relabel a previous Region as missing, and the panel offers no mutation
route.

Issue #1493 completes the Live Inspector's fifth domain. One reading per player
hull lists every built-in Viewscreen mode and every loaded `camera_*` marker,
including the marker position/direction and exact model, rig sidecar, template
and layer provenance retained at runtime. The same reading exposes every typed
view, title-card, Comms-takeover and sound action field plus the current forced
view/card deadline and only the message id, sender and recipient needed to
explain eligible Comms choices. A second reading family lists every captured
sound catalogue, asset, cue and equivalent leaf. Definitions, models, rigs and
assets are Recreate required. Named rows reveal and focus the existing
Presentation panel; they never submit an Inspector action. Sound playback stays
occurrence-only, so the reading explicitly reports that no active-sound history
is retained. Operator profiles, mixer/autoplay state and bridge output devices
never enter the projection. Browser and native use the same panel, bounded
...
an action that does not point at an established typed owner. It reads descriptor
tables only: an unknown extension key appearing in a runtime reading remains
ignored and cannot become an accidental generic setter. The domain schema probes
catch a newly known field without a descriptor. Exhaustive `EntityConfig`,
`WorldConfig`, mission-record, Region-variant, Presentation-cue and sound-record
patterns extend through every nested authored Ship/Station/System config and
every structured Scenario payload row, turning an added Rust field or enum
variant into a compile failure. The union of every shipped hull separately
exercises the composed optional sections and System kinds that can be active.
An exact cross-domain count and full-row fingerprint
then require the reviewed coverage ratchet to move when a descriptor or its
...
join-request and manual-save nodes in place. Closing the workspace restores each
moved node and its authored attributes without cloning listeners. Reusable dock
structure is scoped beneath `.workshop-dock-root` in `gui/dock-layout.css`, while
Workshop's page-global appearance remains isolated in `gui/workshop.css`.

The Live desk uses compact desktop chrome: categorised Session, Crew,
Communications, World and Inspect menus replace the permanent all-panel button
wall. Each dock group has one tab row, with the selected panel's icon-only
Float and Close controls on that same row; it does not repeat the selected tab
in a second header. Dock targets appear only after the pointer has travelled
far enough to become a drag, never on an ordinary tab click. The native map
also measures its canvas synchronously whenever a dock tab reveals it, with a
render-loop size check as an embedded-WebKit fallback, so a missed
`ResizeObserver` transition cannot leave a live projection drawing into a
zero-sized canvas.

Before launch, the roster also lists every authored player-ship slot. An empty
slot offers **Spawn AI ship**, a typed `BackfillShipSlot` GM action settled in
the canonical journal on the stopped lobby boundary. It adds the slot's default
hull to `FrozenShipSlots`; duplicate presses are no-ops and unknown slots are
refused. GameStart remains the only code that spawns the ship and its ordinary
Station backfill.
...
authentic Station consoles. Contacts can differ because of sensor range,
tag filtering, Reveal/Conceal and reported classification. Both Objective comparison columns use the selected ship's
...
The observing Sensors blackboard carries only resolved labels for that ship;
the ordinary Sensors builder substitutes the reported name/classification, and
the Sensors Viewscreen substitutes its contact label. Detection, entity truth,
physical properties, faction, independently earned scans, Comms and Objectives
remain unchanged. A classification does not reveal an absent contact, and
Conceal still wins. The canonical map survives snapshot/reconnect, contributes
to the digest, and is cleared with either identity or a new run. Despawn undo
...
cannot acquire it. The shared Crew Knowledge comparison includes the report as a
crew-only row. Other observers, entity truth, faction and physical state receive
...
Presentation's **Play sound** control uses the same typed `GmAction::Presentation`
route as its other cues. The selected receiving ship must be live; the cue must
be a validated Viewscreen definition. A source contact optionally links the
cue to the receiver's existing conceal/delayed-report policy. Static authored
signals carry no contact link. A successful command keeps ordinary attribution
and result feedback even if the room output is unavailable or muted.

Scenarios use `type = "presentation"` with `presentation.sound = { id = "weapons",
source = "raider" }`, or `ctx.effects.sound("player", "weapons", "raider")` in
Rhai. An empty Rhai source selects a static signal. The source resolves through
existing authored entity names; playback reads the catalog captured at world
ingest, never a file on each command. Reconnect, restoration and journal review
do not replay earlier cues. Local audition remains a separate private preview.
