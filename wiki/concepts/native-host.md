...
2. **A console the LAW seats has no other home.** If `BridgeLayout` seats a
   ...
   `reconcile_seated_consoles` then repairs it boundedly or surrenders the seat
   with a notice, so "not built" cannot mean "a station card claiming a black
   screen forever".
