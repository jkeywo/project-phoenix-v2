...
//
// Unlike reconnect.spec.ts (which simulates a browser *refresh* — the client
...
function consoleSystemState(page: import('@playwright/test').Page) {
  return page.evaluate(() => {
    const state = (window as any).simState;
    const buildConsoleState = (window as any).buildConsoleState;
...
test('sever + revive mid-game: seat restored and console reflects current system state', async ({ context }) => {
  ...
  // midgame-claim-buttons.spec.ts asserts on. Solo Captain lands on the
  // captain console.
  ...
  const { myPeerId, myToken } = await client.evaluate(() => {
    ...
    (window as any).__recordPostReconnectState = false;
    (window as any).__postReconnectStateMessages = [];
    const cm = (window as any).connectionManager;
    const originalSend = cm.send.bind(cm);
    cm.send = (type: string, data?: unknown) => {
      (window as any).__sentMessages.push({ type, data });
      if (type === 'Identify') {
        (window as any).__recordPostReconnectState = true;
      }
      return originalSend(type, data);
    };
    const simState = (window as any).simState;
    const originalApply = simState.apply.bind(simState);
    simState.apply = (msg: any) => {
      if (
        (window as any).__recordPostReconnectState &&
        (msg?.type === 'BlackboardUpdate' || msg?.type === 'SystemHullUpdate' || msg?.type === 'SimState')
      ) {
        (window as any).__postReconnectStateMessages.push({
          type: msg.type,
  ...
  await client.evaluate(
    ({ myPeerId, hostId }) => {
      (window as any).__peerjsShim.severConnection(myPeerId, hostId);
    },
    { myPeerId, hostId },
  );
  ...
  await client.evaluate(
    ({ myPeerId, hostId }) => {
      (window as any).__peerjsShim.reviveConnection(myPeerId, hostId);
    },
    { myPeerId, hostId },
  );
  ...
  await client.waitForFunction(
    () => (window as any).__sentMessages?.some((m: any) => m.type === 'Identify'),
    undefined,
    { timeout: 10_000 },
  );
  const identifyCalls = await client.evaluate(
    () => (window as any).__sentMessages.filter((m: any) => m.type === 'Identify'),
  );
  ...
  await client.waitForFunction(
    () => (window as any).__postReconnectStateMessages?.some(
      (m: any) => m.type === 'BlackboardUpdate' || m.type === 'SystemHullUpdate',
    ),
    undefined,
    { timeout: 5_000 },
  );
  ...
  await client.waitForFunction(
    (token) => {
      const players = (window as any).lobbyState?.players ?? [];
