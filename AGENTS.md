...
7. **AI decisions run on fixed ticks, not frames.** Helm commands apply the tick they are admitted (`AdmittedCommands` is cleared and refilled at admission each tick). **Every** AI policy host — the six per-axis helm systems, shield focus, power allocation, torpedo load/auto-fire, frequency hint, phaser and blaster auto-fire, AI target selection, Captain, Sensors — runs under `run_if` on the one shared cadence in `src/ai/cadence.rs` (`[global] ai_tick_hz`, default 30 Hz; the slower `ai_snapshot_hz` cadence is derived from it as a whole number of base ticks), never once per rendered frame. `SimSet` is configured in Bevy's `Update`, so an ungated decider *is* a per-frame decider. Never gate a decider inside its own body with an `Option<Res<_>>` that falls back to running every tick: every bare-`App` fixture takes that arm, so the shipped cadence ends up covered by no test at all (issue #889).
