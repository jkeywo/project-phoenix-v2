...
test('SimState is broadcast to all clients within 2 s of game start', async ({ context }) => {
  ...
  const simA = await captain.waitForMessage('SimState', 2_000);
  const simB = await helm.waitForMessage('SimState', 2_000);
  ...
  const bbCap = await captain.waitForMessage('BlackboardUpdate', 2_000);
  const bbHelm = await helm.waitForMessage('BlackboardUpdate', 2_000);
  const hasHelm = (updates) => updates.some(([id]) => id === 'helm');
...
test('SetThrust changes ship position in subsequent blackboard updates', async ({ context }) => {
  ...
  const first = await helm.waitForMessage('BlackboardUpdate', 2_000);
  const firstHelm = first.data.updates.find(([id]) => id === 'helm');
  const initX = firstHelm[1].data.x;
  const initZ = firstHelm[1].data.z;
