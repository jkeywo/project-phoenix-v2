...
**Scenario** — a TOML file declaring everything the World should contain: initial entities (asteroid fields, anchors, regions, stations), triggers (`on_attacked`, `on_destroyed`, `on_hailed`, `on_timer`), actions (`load_scenario`, `add_objective`, push comms message), comms exchanges, and objectives. Scenarios are the *only* way content enters the World; there is no separate "world setup" path. The bootstrap scenario is referenced from the map config and loaded at game start; `setup_world_hardcoded` is a fallback scenario embedded in code. A scenario owns the entities, objectives, and messages it created and cleans them up when unloaded. Consumed by the unified `WorldPlugin` (`src/world/server.rs`); see the World vocabulary below.

**World (the plugin / the place)** — the unified server-side substrate that owns everything spatial: entity spawning, asteroid streaming (the `AsteroidWindow` ring buffer + lifecycle), region containment, scenario loading + trigger evaluation, objective tracking. There is no separate Scenario plugin; scenarios are content files the World consumes. `WorldPlugin` lives at `src/world/server.rs`. The `World Data` wire snapshot below is the broadcast view of this state.
