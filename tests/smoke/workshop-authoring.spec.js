...
test('offline Workshop imports, edits, undoes and exports one source-preserving pack', { tag: '@core' }, async ({ page }, testInfo) => {
  ...
  await expect(page.locator('.workshop-dock-panel')).toHaveCount(3);
  ...
  await page.mouse.up();
  const filesBox = await page.locator('[data-panel="files"]').boundingBox();
  const sourceBox = await page.locator('[data-panel="source"]').boundingBox();
  expect(filesBox.y).toBeGreaterThan(sourceBox.y);
  await page.locator('[data-panel="inspector"] .workshop-panel-tab').focus();
  await page.keyboard.press('Control+Shift+ArrowLeft');
  await expect(page.locator('[data-panel="inspector"] .workshop-panel-tab')).toBeFocused();
  await page.locator('[data-panel="inspector"] .workshop-panel-header button').last().click();
...
test('Workshop uses the real runtime for Rhai, source-span fields and browser recovery', { tag: '@core' }, async ({ page }) => {
  ...
  await page.locator('[data-panel="inspector"] summary').click();
