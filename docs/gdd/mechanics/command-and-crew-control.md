...
| Scope | Command stances, AI station direction, spectators, AFK delegation and human-seeking placement |
...
Related documents: [AI and Backfill](../systems/ai-and-backfill.md), [Station Experiences](../systems/station-experiences.md), [Game and Session Lifecycle](../foundation/game-lifecycle.md), [Onboarding and Accessibility](../foundation/onboarding-accessibility.md), and [Planned but Not Scheduled](../future/planned-not-scheduled.md).

## Command system

Command is a human-seeking system normally assigned to Captain through hull data, never by a hard-coded station rule. It lists AI-controlled stations as aggregate units and exposes the stances currently available to each. It does not operate individual systems, bypass station admission or grant the operator a second set of console controls.
...
## Station ownership and system delegation

A station is player-owned or AI-owned. Handoff remains system-level: a player-owned station may delegate some or all of its systems to AI without losing the station claim. Another player cannot take a system merely because it is automated.

An AFK setting temporarily delegates every system on the player's station while retaining the seat and its previous control configuration. The AFK player stops qualifying as a destination for human-seeking systems, which relocate through their authored preference order or fall back to AI. Returning restores the station's previous system controls and recomputes human-seeking placement; visiting systems do not blindly snap back.
...
Later accessibility assistance builds on the same delegation and human-seeking rules. Player settings describe functional effects, not diagnoses, and remain private. Other players may see that a function is AI-assisted or that a station is unsuitable, but never the reason a player enabled an option.
...
- AFK delegates systems without relinquishing the station and relocates human-seeking systems safely.
...
- Does AFK preserve a player's seat while keeping the ship and visiting systems usable?
