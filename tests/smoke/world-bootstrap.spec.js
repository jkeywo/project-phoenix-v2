...
// fixture routes that path to `MINIMAL_DEFAULT_WORLD` (see fixtures.js), which
...
test('default scenario: the world\'s station appears in WorldSetup after game start', async ({ context }) => {
  ...
  expect(
    starbase,
    `Expected a station entity in WorldSetup. Got: ${JSON.stringify(entities.map((e) => ({ id: e.id, tags: e.tags })))}`,
  ).toBeDefined();
