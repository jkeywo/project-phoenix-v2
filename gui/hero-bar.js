...
export function heroBarModel({ directStation, stations, stationSystems,
  blackboards, stationHosts, stationRatings, activeStation }) {
  const defs = stations || [];
  ...
  return {
    ...
    tabs: tabIds.map(id => {
      ...
      const rating = mode === 'visiting'
        ...
        : (stationRatings?.[id] || '');
      return { id, name: st.name || id, mode, rating, selected: id === selected };
    }),
    ownership,
    aiStations: defs
 ...
 */
export function renderHeroBarDom({ tabsEl, titleEl, metaEl, aiEl, model,
