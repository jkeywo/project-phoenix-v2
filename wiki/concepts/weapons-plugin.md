...
Above that host gate sits the per-tube launch **policy** (`torpedo_launch` channel). Its fact snapshot is seeded by `seed_torpedo_tube_launch_facts` (`console/weapons/torpedo.rs`) and, since issue #791, carries a ship-wide `tubes_full` reading — every tube at `loaded_count == volley_max` — alongside the per-tube `loaded` (which is only `loaded_count > 0`). The two answer different questions: `ship_harrow_cruiser` authors a salvo doctrine that fires all its tubes into one shield gap or none of them, and that needs the stronger reading. `target_facing_shields` beside them is an HP value, not a boolean.
