...
- **`snapshot`** (#1117) — one framed piece of the **portable authoritative record**. The transfer is the transport half of `p2p-delta-snapshot-is-whole-payload-ron`: chunk / reassemble / integrity around the ONE `PhoenixSnapshot` record `src/snapshot.rs` already produces, no second serializer.
  ...
  - **Adapter + gate:** `src/lockstep/snapshot_relay.rs`. `capture_run`/`frames_for` reuse `snapshot::{capture,run_for,export_artifact}`; `gate_and_restore` reuses `snapshot::import_artifact` (the `Versions::check` build/rules/content-digest gate, run **before** any state is written) then `snapshot::restore` then the `world_digest` post-restore corruption check — the same seams a local file import uses. A refused record leaves the receiving world untouched.
  - **Isolation (AC4).** The store rides whole; what a crew SEES is gated by the `LocalShip`-scoped projection layer (`src/dossier/server.rs`), and `LocalShip` is host-local and never in the record — so a restore cannot re-point projection at another crew. True per-ship evidence KEYING is open PRD #1016's (`p2p-delta-per-ship-epistemics`).
