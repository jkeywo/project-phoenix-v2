...
describe('PhShieldPanel', () => {
  ...
  it('renders default state with full hull', () => {
    const { el } = setup();
    el.state = {};
    expect(queryText(el, '#hull-val')).toBe('100%');
    expect(queryText(el, '#grid-status')).toBe('GRID NOMINAL');
    expect(queryText(el, '#focus-display')).toBe('FOCUS: OMNI');
  });
  ...
  it('renders shield facings from state', () => {
    ...
    expect(queryText(el, '#hull-val')).toBe('78%');
  });
  ...
  it('updates when state changes', () => {
    ...
    expect(queryText(el, '#hull-val')).toBe('90%');
    el.state = { hull_integrity_pct: 45 };
    expect(queryText(el, '#hull-val')).toBe('45%');
  });
