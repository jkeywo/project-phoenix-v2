...
test('nav chart shows non-objective entities through the real page pipeline', async ({ context }) => {
  ...
  // reconnect-midgame-sever.spec.js).
  ...
  await client.waitForFunction(
    () => ((window.simState?.navChartShows) ?? []).length > 0,
    undefined,
    { timeout: 10_000 },
  );
  ...
  const blip = await client.evaluate(() => {
    const nav = JSON.parse(
      window.buildConsoleState('navigation', window.simState),
    );
    return nav.blips.find((b) => b.name === 'Starbase Alpha');
  });
