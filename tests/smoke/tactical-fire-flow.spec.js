...
# than authoring them under \`overrides\` the way patrol.spec.js does — the same
...
async function startGameWithTactical(context) {
...
test('tactical fire-flow: BeamStarted received after locking NPC and firing', async ({ context }) => {
  ...
  const beamStarted = await tactical.waitForMessage('BeamStarted', 15_000);
...
test('tactical fire-flow: NPC hull_fraction decreases after phaser hit', async ({ context }) => {
  ...
  const initialHull = await tactical.page.evaluate((uuid) => {
    const msgs = window.__messages || [];
    for (let i = msgs.length - 1; i >= 0; i--) {
      const m = msgs[i];
      if (m.type === 'SimState') {
        const es = m.data.snapshot.entity_states || [];
        const entry = es.find((e) => e.uuid === uuid);
        if (entry?.hull_fraction !== undefined && entry.hull_fraction !== null) {
          return entry.hull_fraction;
        }
      }
    }
    return 1.0;
...
test('tactical fire-flow: EntityDespawned received when NPC hull reaches 0', async ({ context }) => {
  ...
  await tactical.page.evaluate(() => {
    window.__fireInterval = setInterval(() => {
      const msgs = window.__messages || [];
      const despawned = msgs.some((m) => m.type === 'EntityDespawned');
      if (despawned) { clearInterval(window.__fireInterval); return; }
      // Find the most recent WeaponsUpdate and pick the first fire-ready bank.
      let readyBank = null;
      for (let i = msgs.length - 1; i >= 0; i--) {
        const m = msgs[i];
        if (m.type !== 'WeaponsUpdate') continue;
        const banks = Array.isArray(m.data.banks) ? m.data.banks : [];
        const found = banks.find((b) => b.fire_ready === true);
        if (found) { readyBank = found.id; }
        break;
      }
      if (readyBank) {
        window.__conn.send(JSON.stringify({
          type: 'ControlSystem',
  ...
  await tactical.page.waitForFunction(
    (uuid) => window.__messages?.some(
      (m) => m.type === 'EntityDespawned' && m.data.uuid === uuid,
    ),
    raiderUuid,
    { timeout: 90_000 },
  );

  await tactical.page.evaluate(() => clearInterval(window.__fireInterval));

  // Verify the despawn message is present.
  const despawnMsg = await tactical.page.evaluate((uuid) => {
    return window.__messages.find(
      (m) => m.type === 'EntityDespawned' && m.data.uuid === uuid,
    );
  }, raiderUuid);
