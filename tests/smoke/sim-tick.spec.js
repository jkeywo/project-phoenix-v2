...
test('sim advances on the fixed logical tick, not the rendered frame', async ({ context }) => {
  ...
  await serverPage.evaluate(() => window.wasm_toggle_pause());
  // One frame for the PreUpdate drain to apply the toggle, then measure.
  await waitFrames(serverPage, 5);
  const pausedStart = await serverPage.evaluate(() => window.wasm_sim_tick());
  await waitFrames(serverPage, 60);
  const pausedEnd = await serverPage.evaluate(() => window.wasm_sim_tick());
  expect(
    pausedEnd,
  ...
  const resumed = await serverPage.evaluate(() => window.wasm_sim_tick());
