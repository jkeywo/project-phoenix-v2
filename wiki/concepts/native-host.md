...
The host lobby's GM row assigns one dedicated monitor, excluded from viewscreen
and station occupancy and retained with the bridge layout. The role is selected
before launch; during a mission the host may move it to a free monitor. Its
operator participates in the existing readiness and Force Start policy.

`native_gm` owns a private surface bridge and lifecycle alongside the ship.
`gui/gm-workspace.js` shares the full browser presentation;
`gui/native-gm-workspace.js` binds it to existing typed GM actions and local
projections. The document never boots another WASM simulation and never joins
the crew pane transport. Its distinct operator is attributed through the GM
journal without creating a fleet peer or removing the local ship.

Screen failure and monitor loss retain identity and pause a running simulation
once. Surface rebuilding is bounded; recovery leaves the pause in place until
an explicit GM Resume. The private bridge refuses an overflowing reliable
batch as a surface failure rather than applying commands after lost records.

Native private feedback is owned by `audio/private.rs`, connected to the real
Station `pump_pane` and `NativeGmBridge` adapters. The host binds each current
pane incarnation to its Station or participant media label; `native-gm` is the
private GM media surface. Those labels cannot collide with another active
console or the reserved room/GM labels. Add explicit `[[media]]` outputs in the
existing bridge profile; a missing assignment stays silent. Settings names the
surface/output and exposes a bounded Test output control, subject to Master and
Interface. Independent cursors support multiple deliberately assigned outputs.

The room worker reports its actually opened default device for sharing checks.
An unresolved default requires an explicit room selection before private sound
can play; a known overlap uses ordinary `allow_shared` consent. No private
provider calls system-default selection. Missing, ambiguous or failed outputs
stop active sound and require deliberate retry of the same assignment.

The shared `panes/operator_storage.js` waits for the established native operator
profile before sending private sound. Station files retain hull/name binding;
GM files use the same store under the host-chosen `native-gm:<hull>` scope and
`native-gm` name, without a crew identity. The private bridge transports current
mix/control and one expiring cue outside reliable action queues. The shared JS
semantic/alert owner supplies meaning; Rust does not recompute alert eligibility
or store audio history. Rebuild, restore and output recovery do not replay cues.

`audio/private_audition.rs` adds one deliberate GM preview to that same owner.
Its typed request carries one validated packaged sound and asset declaration;
the host still binds the endpoint and output assignment. Production decoding
and optional authored-direction HRTF preparation happen outside endpoint locks,
then the current generation and preview identity are checked before playing.
The two-second preview uses its declared category, the private Master/category
mix and installed comfort processing. Stop, mute, output loss, page retirement
and restore discard it. It never enters the GM action queue or narration.
The shared GM panel renders authored meaning/source/urgency/direction even when
sound is muted or absent; it does not enrich them from live entity truth.
...
The observer itself does not change application policy.

**HUD revisions.** `panes/hud.rs` retains the encoded command for the newest
changed HUD value. The adapter sends only changed revisions, and `PaneLoop`
tracks each HUD view's last successful application separately from a refresh
owed after load/recreation, reveal or resize. A failed push retries without
claiming success; copy failures and staging starvation retain their own full-copy
...
`viewscreen-presentation.toml` with visual preferences, preserving each section
through the other's writes and resets. Output assignment uses bridge-media
vocabulary in `bridge-media.toml`, separate from endpoint preferences. An explicit
...
Reduced dynamic range uses the same endpoint/private preference ownership and
shared control. `audio/range.rs` runs a linked peak/gain envelope after the
authored, spatial, mono, category and room-ducking stages and before final Master.
Its canonical tuning is `assets/audio/reduced-range.toml`; the browser projection
drives a real Web Audio compressor through `gui/audio-range.js`. Full dynamics
is the default bypass; neither live toggle restarts sources. Native and browser
detectors are independently implemented, not waveform-identical. Both bound
peaks, preserve final Master zero and discard stale playback at lifecycle/mute
boundaries. The browser rebuilds its processor to discard lookahead on mute.
`audio/range_tests.rs`, the paired private-adapter sample test and
`tests/smoke/audio-range.spec.js` cover samples, live controls and persistence;
human listening remains separate acceptance work.
...
The room Audio panel also offers optional alert ducking, off by default.
`audio/ducking.rs` reads `assets/audio/room-ducking.toml`; `Mixer::important_cue`
starts one bounded sample-clock envelope for an audible important Alert.
Only Music/Ambience samples are attenuated, with smooth recovery under overlap
and when disabling the option. Ordinary private cue callers remain independent.
The choice persists and resets with the endpoint audio section; output or
continuation reset discards its transient window.
...
- **Escape reaches the focused page.** The SDK-independent Bevy keyboard
  ...
  its ordinary listener. The shared [focus trap](../../gui/focus-trap.js) also
  recognises Ultralight's legacy Escape code (`27` / `U+001B`) when the named
  key is missing or `Unidentified`; a different valid named key takes priority.
  The [captured-event fixture](../../tests/fixtures/native-escape-keydown.json)
  drives the real Settings component's dismissal and focus restoration test.
  F9 remains host-owned. Text, modifiers and key-release behavior retain the
  adapter's existing limits.
...
errors. Tokens and station authority never enter these files.

Bevy supplies physical gamepad snapshots through `panes::gamepad`.
...
The shared settings, remapper and help read this same active controller state.
Helm's default-on hide preference independently replaces its joystick and lateral
control only when their selected controller bindings are usable. Both return
when their replacement stops being available. See the hardware checks in
[the acceptance kit](../../docs/acceptance/controllers-native-gm.md).
...
The matching scenario is the peer GM's entire content choice: its picker does
not ask for a slot or hull, because the ship-owning peer supplies that topology.
The GM desk resolves its local operator from the admitted fleet binding rather
than assuming the standalone `native-gm` id, so actions remain enabled after
admission.
...
The screen uses an ordinary per-console session token and client document,
with an additional host-owned station reservation. `SessionManager` binds that
token to its assigned station and refuses switching, voluntary release and
competing claims. A connected phone must release its station before the host
can assign its screen. Named participant panes retain ordinary selection rules.
...
station flips to `Backfill`. `ConsoleAssignments` retains the desired station,
monitor and session identity independently of this physical layout. The
reservation survives unplug and recovery; host Off releases it, and a host move
restores it on the same identity. The lobby retains Off and move controls when
the desired monitor is absent.
...
crash is held to. When the budget is spent, the physical seat is removed through
the law (`UnassignStation`) with a visible notice. The separate host assignment
retains station authority and session identity, with Backfill operating until
the host moves or closes the screen. On the pane host's side, a failed
...
that token identifies again. Host-assigned screens keep an admission reservation
throughout that page load, so another client cannot take their station. Named
participant panes keep the ordinary reconnect rule, conditional on no other
connected holder. The adapter tells
...
### Live authored sounds

The GM Presentation panel and scenario presentation actions name a validated
Viewscreen cue from `assets/audio/sound-cues.toml`. World ingest captures its
exact source once and includes it in content identity, including headless and
disposable Test boots. The native room owner receives the accepted cue through
the existing audio channel and uses the declared category, authored gain,
listener-relative direction and current comfort settings. Private endpoints do
not acquire room playback.

One expiring current request survives only while its lifecycle generation,
output request and source definition remain current. Duplicate, muted, late and
restored deliveries are consumed silently; output recovery cannot replay them.
Informative text uses the existing HUD live-equivalent row independently of the
audio device. An optional source contact applies the shared M7 suppression rule
