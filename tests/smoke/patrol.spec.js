...
// self-contained fixture (below), the same convention `tactical-fire-flow.spec.js`
// uses; see the header of `fixtures.js` for where every fixture world lives.
...
description = "Player ship + one NPC raider; see tests/smoke/patrol.spec.js."
...
test('a world-authored NPC appears in WorldSetup after game start', async ({ context }) => {
  ...
  const worldSetupMsg = await helm.waitForMessage('WorldSetup', 5_000);
  ...
  expect(
    raider,
    `Expected the fixture's NPC (tag: npc) in WorldSetup.world.entities. Got: ${JSON.stringify(entities.map((e) => ({ id: e.id, tags: e.tags })))}`,
  ).toBeDefined();
