...
//! * **The non-colour focus indicator** ([`FocusReticle`], [`FocusReticleStyle`])
//!   — the geometry of the bracketed focus frame, and how the OS contrast and
//!   reduced-motion preferences change it. Acceptance criterion 3's "visible
//!   without relying on colour" and acceptance criterion 4's "contrast and
//!   reduced-motion apply to native setup chrome": the reticle is drawn by the
//!   feature-gated adapter ([`super::panes::ultralight`]) from exactly this
//!   descriptor, so the shape and the contrast response are CI-tested here rather
//!   than only on the dev box.
...
/// How the keyboard-focus reticle is drawn, once the OS accessibility
/// preferences are folded in.
///
/// These are the dimensions of an accessibility affordance in a pane's own
/// **logical** pixels, so they read the same at any monitor scale — not gameplay
/// values and not designer tunables, the same footing as the reticle constants
/// [`super::panes::ultralight`] drew before this owned them. The indicator is a
/// full frame **plus** four corner brackets ([`FocusReticle::BRACKET_CORNERS`]),
/// drawn on exactly the focused pane and no other, so focus is shown by the
/// *presence of that shape*, never by a colour — a colour-blind operator, or one
/// at a washed-out bridge display, sees the bracketed frame appear (WCAG 1.4.1).
///
/// The OS contrast preference (acceptance criterion 4, native setup chrome) makes
/// the frame **bolder and fully opaque**; it never becomes the *only* cue,
/// because an unfocused pane still has no frame at all. The reduced-motion
/// preference is honoured by construction: the reticle never animates — it is
/// spawned and despawned as focus moves, with no transition to suppress — so
/// [`FocusReticleStyle::for_prefs`] carries `reduced_motion` only to record that
/// the decision was consulted, and [`FocusReticleStyle::animates`] is always
/// `false`.
#[derive(Clone, Copy, Debug, PartialEq)]
pub struct FocusReticleStyle {
    /// Border thickness of the frame and brackets, logical pixels.
    pub thickness_px: f32,
    /// Arm length of each corner bracket, logical pixels — long enough to read as
    /// a deliberate reticle rather than a rounded corner.
    pub bracket_px: f32,
    /// Inset of the frame from the pane edge, logical pixels, so it sits just
    /// inside the console rather than being clipped at the window edge.
    pub inset_px: f32,
    /// The alpha the adapter draws the (white) reticle at. Full under high
    /// contrast, near-opaque otherwise. The hue is never load-bearing — see the
    /// type note.
    pub alpha: f32,
}

impl FocusReticleStyle {
    /// Standard-contrast thickness, logical pixels.
    pub const STANDARD_THICKNESS_PX: f32 = 3.0;
    /// High-contrast thickness — a bolder frame for `prefers-contrast: more`.
    pub const CONTRAST_THICKNESS_PX: f32 = 4.0;
    /// Corner-bracket arm length, logical pixels.
    pub const BRACKET_PX: f32 = 26.0;
    /// Frame inset from the pane edge, logical pixels.
    pub const INSET_PX: f32 = 2.0;
    /// Standard-contrast alpha — near-opaque white, matching the viewscreen HUD.
    pub const STANDARD_ALPHA: f32 = 0.92;
    /// High-contrast alpha — fully opaque.
    pub const CONTRAST_ALPHA: f32 = 1.0;

    /// The style with no accessibility preference in force.
    pub fn standard() -> Self {
        Self {
            thickness_px: Self::STANDARD_THICKNESS_PX,
            bracket_px: Self::BRACKET_PX,
            inset_px: Self::INSET_PX,
            alpha: Self::STANDARD_ALPHA,
        }
    }

    /// The style under the given OS accessibility preferences (acceptance
    /// criterion 4). High contrast thickens the frame and takes it fully opaque;
    /// reduced motion is a no-op the reticle satisfies by construction (it never
    /// animates), consulted here so the wiring is explicit rather than absent.
    pub fn for_prefs(high_contrast: bool, reduced_motion: bool) -> Self {
        let _ = reduced_motion; // the reticle never animates; see the type note.
        let mut style = Self::standard();
        if high_contrast {
            style.thickness_px = Self::CONTRAST_THICKNESS_PX;
            style.alpha = Self::CONTRAST_ALPHA;
        }
        style
    }

    /// The style for a whole [`OsAccessibilityPrefs`] read.
    pub fn for_os_prefs(prefs: &OsAccessibilityPrefs) -> Self {
        Self::for_prefs(prefs.high_contrast, prefs.reduced_motion)
    }

    /// Whether the reticle animates its transitions. Always `false`: the focus
    /// indicator is static so it needs no reduced-motion special case, which is
    /// itself the reduced-motion guarantee for host-drawn setup chrome.
    pub fn animates(&self) -> bool {
        false
    }
}

/// A rectangle of the focus reticle in a pane's own **logical** pixels, origin at
/// the pane window's top-left.
#[derive(Clone, Copy, Debug, PartialEq)]
pub struct ReticleRect {
    pub left: f32,
    pub top: f32,
    pub width: f32,
    pub height: f32,
...
/// The geometry of the keyboard-focus reticle over one focused pane — the frame
/// and the four corner brackets, in logical pixels.
///
/// The feature-gated adapter ([`super::panes::ultralight`]) turns this into Bevy
/// UI nodes; keeping the arithmetic here means the shape (a frame **and** four
/// brackets, present only when focused) is proven by the ordinary CI runs rather
/// than only under the ignored GPU test.
#[derive(Clone, Copy, Debug, PartialEq)]
pub struct FocusReticle {
    /// The inset frame rectangle.
    pub frame: ReticleRect,
    /// Border thickness of the frame and brackets, logical pixels.
    pub thickness_px: f32,
    /// The side length of each of the four corner brackets, logical pixels.
    pub bracket_px: f32,
    /// The alpha the (white) reticle is drawn at.
    pub alpha: f32,
}

impl FocusReticle {
    /// The reticle is a bracketed frame — a full border plus this many corner
    /// brackets — so focus reads by shape, not colour.
    pub const BRACKET_CORNERS: usize = 4;

    /// The reticle over a pane whose **logical** box has top-left
    /// `(origin_x, origin_y)` and size `(width, height)`, styled by `style`.
    ///
    /// The frame is inset on every side by [`FocusReticleStyle::inset_px`]; a pane
    /// smaller than twice the inset yields a zero-size frame rather than a
    /// negative one, matching the adapter's `.max(0.0)` guard.
    pub fn for_pane(
        origin_x: f32,
        origin_y: f32,
        width: f32,
        height: f32,
        style: &FocusReticleStyle,
    ) -> Self {
        let inset = style.inset_px;
        Self {
            frame: ReticleRect {
                left: origin_x + inset,
                top: origin_y + inset,
