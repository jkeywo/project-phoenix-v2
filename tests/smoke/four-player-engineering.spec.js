...
// The hull `MINIMAL_DEFAULT_WORLD` spawns as the player ship (see fixtures.js).
...
// `sim-state.spec.js`, per issue #941.
...
async function lastAssignment(client, token) {
  return client.page.evaluate(
    (t) => {
      const msgs = window.__messages || [];
      const all = msgs.filter((m) => m.type === 'StationAssigned' && m.data.token === t);
...
async function waitForLastMessage(
  ...
  await client.page.waitForFunction(
    ({ type, predicate }) => {
      const msgs = window.__messages || [];
      const last = msgs.filter((m) => m.type === type).pop();
...
async function setHelmPower(client, level) {
 ...
 */
async function buildFourPlayerCrew(context) {
...
test('shared session-token orphans the first Engineering device (ghost console)', async ({ context }) => {
  ...
  const preCount = await c3.page.evaluate(() => window.__messages?.length ?? 0);
  ...
  const sawGhostChange = await c3.page.evaluate(
    ({ count, level }) => {
      ...
      return newMsgs.some((m) => m.type === 'PowerState' && m.data?.helm === level);
    },
