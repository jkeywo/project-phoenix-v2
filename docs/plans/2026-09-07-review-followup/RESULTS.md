...
P2 first paint, duplicate suppression and steady upload reduction are evidenced; the corrected SDK run also demonstrated genuine red-alert changes and F9 reveal/hide. A real mission ending exposed missing Station terminal delivery and empty HUD ending text. The lifecycle dispatch/ordering correction is integrated at 08b8598d, with eight focused Rust checks passing; the old WASM reproduces both faults in a real renderer regression. Updated SDK/browser acceptance, heading/hull changes and recovery remain pending. A main-frame cadence near 60 Hz is neither console 60 Hz nor an input-latency result. No issue closure, three-Station acceptance, GPU-memory result or general percentage speedup is asserted here.
