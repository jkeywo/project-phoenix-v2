...
// a second opinion written in TypeScript.
//
...
// SwiftShader route the *.capture.ts aids use, which measures a software
...
test('browser host: boot, preload and frame time are measured', async ({ context }) => {
  ...
  const json = await serverPage.evaluate(
    (scenario) => (window as any).wasm_perf_capture(scenario) as string,
    SCENARIO,
  );
