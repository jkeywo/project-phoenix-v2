...
`pane_rects` call: the authored surfaces first (they were there from boot, so
they keep the left half), then the seats in seat order.
`follow_layout_stations` writes that list over the surface wholesale rather than
tiling the stations around the authored panes. Two tilings, each handed the full
rectangle, *was* the bug; one tiling over a count rule 3 already bounds is the
fix, and it invents no new tiler. `station_rects` is that same tiling filtered,
not a tiling of its own.
...
that it is no longer silent. The adapter tells the two apart by the evidence it
already has — a console whose **monitor** changed was moved by the press the
operator just made, one whose **rectangle** changed where it stood was re-tiled
by a neighbour — and raises `LayoutAdoption::ConsoleRetiling` on the lobby row
for the second. It names the console rather than a `StationId`, because an
authored `--pane` participant is re-tiled by the same rule. The operator's own
move stays silent: narrating a press back to the person who made it buries the
line that is news.
...
the same `occupants_on` list the monitor button draws and the refusal names. That
matters most for the occupant no station row can ever show: a console a
`--profile` opened for a named crew member, which fills a slot and has no card.
