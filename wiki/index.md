...
- **[CommsPanel](./concepts/comms-panel.md)** — Client-side plugin for the Comms console UI: `ClientCommsState` initialisation, `CommsView` placeholder resource, panel visibility. Folds in and replaces `comms_plugin.rs`. Lives in `src/comms_panel.rs` (#262).
...
- **[PRD #438 — HTML/JS Client GUI Shell](./sources/prd-438-html-client-gui-shell.md)** — In flight. Replace Bevy lobby + tab bar + phone bezel with HTML/CSS/JS. Slices: #439 (bezel, shipped), #440 (lobby, shipped), #441 (tab bar), #442 (Bevy cleanup).
