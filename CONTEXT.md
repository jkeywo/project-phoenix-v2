...
**Scenario (legacy term)** — historically a separate TOML under `assets/scenarios/` that paired with a map TOML under `assets/maps/`. As of the world-merge step 1, scenarios and maps share a single TOML under `assets/worlds/` (see *World File* below). The Rust types `ScenarioConfig`, `parse_scenario`, and `ScenarioManager` still exist internally because the type-level merger is incomplete — tracked by PRD #337.

**World File** — a single TOML under `assets/worlds/` that declares everything a session contains: anchors, `[[entity]]` instances, named `[[spawn]]` entries (trigger/comms-eligible), `[[trigger]]` reactions, `[[comms]]` dialogue templates, and objectives. One world file per session; chaining (`load_scenario`/`unload_scenario`) is no longer supported. Loaded by JS via `wasm_load_world(path, toml_str)`. The unified `WorldPlugin` (`src/world/server.rs`) consumes it.
...
**WorldPlugin** — the unified server-side substrate in `src/world/server.rs` that owns world-file loading (`wasm_load_world`), entity lifecycle, trigger evaluation, objective tracking, and `WorldSetup` broadcast. World content is loaded via `WorldContentResource` / `WorldContentRuntime`. The JS bridge calls `wasm_load_world(path, toml_str)` directly with a single TOML; the legacy `wasm_get_world_content_path()` round-trip is gone. The old "ScenarioPlugin" name was retired; the file format types (`ScenarioConfig`, `parse_scenario`) retain their names pending the type-level merger tracked by PRD #337.
