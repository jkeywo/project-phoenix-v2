...
| `src/world/layers.rs` | Pure (Bevy-free) world-layer decisions: `evaluate_layer_load` (dedup, parse/compile handling, name→UUID assignment) for the `apply_world_layer_changes` applier (#821, #1045). `Merge` carries compiled ASTs, trigger and deadline handlers into atomic activation. On WASM an `AwaitingScript` change retains the already-fetched TOML until its sibling Rhai is ready; failure refuses the whole layer. The applier registers origin-scoped script state and deadlines, while unload removes that layer's registrations/calls/rows and releases a shared AST only after its last owner. |
