...
pub fn process_message(
    ...
    match msg {
        ...
        ClientMessage::ToggleRedAlert | ClientMessage::HelmInput { .. } | ClientMessage::SetView { .. } => {}
    }
...
mod tests {
    use super::*;
    use crate::shared::messages::{AsteroidInfo, WorldData};
    ...
    fn welcome_during_in_progress_carries_world_some() {
        let mut sessions = SessionManager::new();
        let world = WorldData { asteroids: vec![AsteroidInfo { x: 1.0, z: 2.0, radius: 2.0 }] };
        let msg = ClientMessage::Identify { token: "t1".into(), name: "Alice".into() };
