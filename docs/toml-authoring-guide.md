...
| `narrative` | bool | `false` | Opt this entity into the authored mission timeline (issue #1338). The headless run report then records when it entered the world and when it died, under its `name`. Works with either `spawn_on` value — `"immediate"` and `"game_start"` both mark. Requires a `name` — the mark's payload *is* the name — and a row that sets the flag without one raises a `narrative-mark-needs-name` validation warning instead of being marked. Everything else (escaped, rescued, abandoned, disabled) is your judgement, through `ctx.effects.narrative_outcome(..)`. Declared `[[entity]]` rows only: a hull a script spawns mid-run with `ctx.effects.spawn_entity(..)` has no row to carry the flag, so record its story with `ctx.effects.narrative_outcome("name", "spawned")` and `…("name", "destroyed")` instead — same vocabulary, same timeline. An unmarked hull produces no timeline entry however violently it dies: a story event is authored, never inferred. |
