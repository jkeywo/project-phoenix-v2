...
## Running-mission host-mesh frames (issues #1116, #1117)
...
- **`snapshot`** (#1117) — one framed piece of the **portable authoritative record**. The transfer is the transport half of `p2p-delta-snapshot-is-whole-payload-ron`: chunk / reassemble / integrity around the ONE `PhoenixSnapshot` record `src/snapshot.rs` already produces, no second serializer.
  ...
  - **Adapter + gate:** `src/lockstep/snapshot_relay.rs`. `capture_run`/`frames_for` reuse `snapshot::{capture,run_for,export_artifact}`; `gate_and_restore` reuses `snapshot::import_artifact` (the `Versions::check` build/rules/content-digest gate, run **before** any state is written) then `snapshot::restore` then the `world_digest` post-restore corruption check — the same seams a local file import uses. Only a version/content-gate refusal leaves the receiving world untouched: that gate runs before a single component is written. A refusal decided *after* `snapshot::restore` has run — an integrity mismatch or an incomplete restore — has already overwritten the world, with no rollback until #1118's divergence recovery restores into a checkpoint and swaps only on a matching fold.
