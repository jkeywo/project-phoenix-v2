...
- **A native template-cache check.** `boot::build` runs no preload of its own,
  and six call sites read the native entity-template cache with no filesystem
  fallback — `lobby::server::update_session_with_config` worst of all, which on
  a miss keeps a *default* `ShipClientConfig` and lets a mission run on with the
  wrong radar range and nothing in the log. For this profile boot refuses to
  compose unless every template the world declares
  (`world::config::entity_template_paths`) is already cached.
...
`ShutdownSignal`. A native process also resolves content two independent ways —
...
| File | Claim |
|---|---|
| `src/boot/tests.rs` | Four-profile parity; only the render-stack profiles take that path; a native host refuses a configless boot |
| `src/native_host/transport.rs` | The seam's ingress/egress and the reserved-token refusal |
| `tests/native_host_sim.rs` | Shipped content boots and runs; the hull's own config reaches the client config; a participant joins through the seam; native↔headless digest equivalence |
| `tests/native_viewscreen_render.rs` | The viewscreen draws a non-flat frame on a real GPU. `#[ignore]`d — CI is ubuntu-only with no display |
| `tests/native_host.rs` | The delivery half, unchanged, plus the serving loop's shutdown path |
