 ...
 * itself is NOT written here: it is authored data in
 * `assets/join/join-codes.json`, passed in as `data` or installed once with
 * `setJoinCodeData()`.
 *
...
/** Wire/format revision this module implements. */
 ...
 *
 * A bare suffix (no separators) is resolved against `fallbackNamespace` — that
 * is the ordinary phone entry path, where the page supplies the namespace. A
 * three-part string is taken at its word, including a project GUID this build
 * does not recognise (reported as `unknown-project`, never silently retyped).
 *
...
export function parseJoinCode(raw, fallbackNamespace, data) {
  ...
  const parts = fragment.split(PART_SEPARATOR);

  if (parts.length === 1) {
    const composed = joinCodeForSuffix(parts[0], fallbackNamespace, d);
 ...
 * Reasons come from this module (`validateSuffix`, `parseJoinCode`) and from
 * the rendezvous service's `error` frames (`worker-rendezvous/src/registry.js`).
 */
const REASON_STRING_IDS = {
  ...
  'unknown-project': 'client.join.error_wrong_type',
  'unknown-namespace': 'client.join.error_wrong_type',
