...
Factions are loaded from `assets/factions/*.toml` (`FactionConfig` at `src/ai/faction.rs:17`) into a `FactionRegistry` (`src/ai/faction.rs:53`) exposed as `FactionRegistryResource` (`src/entities/config_cache.rs:999`). The asymmetric `is_enemy(a, b, registry)` predicate (`src/ai/faction.rs:148`) returns `true` only when `a`'s `enemies` list contains `b`; factionless entities are neutral to everyone. Since #1030 a faction may also carry an optional `display_name` — a `strings.csv` id, and the only faction string a player ever sees; `name` stays the reference key `add_faction_enemy` and entity templates resolve by. The AI's shared nearest-hostile scan (`find_nearest_hostile`, `src/ai/core.rs:691`) consults this predicate when picking a target.
