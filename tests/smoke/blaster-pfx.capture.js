...
// stationary raider dead ahead, mirroring tactical-fire-flow.spec.js's
...
// tested) — this is a verification aid, modeled on dust-pfx.capture.js. Run
// it explicitly:
//
//   npx playwright test --config=playwright.capture.config.js blaster-pfx.capture.js --headed
//
...
test('capture blaster bolts firing on a stationary target', async ({ context }) => {
  ...
  // Self-contained world — same rationale as tactical-fire-flow.spec.js.
  ...
  // Chromium throttles hard on backgrounded tabs (see dust-pfx.capture.js) —
  ...
  while (Date.now() < until && !(helmStarted && tacticalStarted)) {
    ...
    helmStarted ||= await helm.page.evaluate(
      () => window.__messages?.some((m) => m.type === 'GameStarted'),
    );
  ...
  const raider = entities.find(
    (e) => Array.isArray(e.tags) && e.tags.includes('npc') && e.tags.includes('enemy'),
  );
  ...
  await tactical.page.waitForFunction(
    () => window.__messages?.some(
      (m) => m.type === 'WeaponsUpdate' && m.data?.target_uuid,
    ),
    undefined,
    { timeout: 10_000 },
  );
  ...
  // tactical-fire-flow.spec.js, then fire (volley_count=3,
  ...
  await tactical.page.waitForFunction(
    () => window.__messages?.some(
      (m) => m.type === 'WeaponsUpdate'
        ...
        && m.data.blasters.some((b) => b.id === 'port' && b.fire_ready === true),
    ),
    undefined,
    { timeout: 15_000 },
  );
